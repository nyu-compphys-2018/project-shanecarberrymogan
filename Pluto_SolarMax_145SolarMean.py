...
class Pluto:
    ...
    def Iterative_Heating_145SolarMean(self, i, n, tau_CH4, tau_N2):
        ...
        for k in range(1, i+1):
            ...
            Q_CH4[k] = 1.45 * 4. * self.pi * self.q_UVmean_const * q_r_CH4
            Q_N2[k] = 1.45 * 4. * self.pi * self.q_EUVmean_const * q_r_N2
    ...
    def convergence_iteration_145SolarMean(self, i_x, n, r, T, phi, phiE):
        i_Kn01 = self.i_Kn01(i_x, n, r, T)
        tau_CH4, tau_N2 = self.Tau(i_Kn01, n)
        Q0_CH4, Qr_CH4, Q0_N2, Qr_N2 = self.Iterative_Heating_145SolarMean(i_Kn01, n, tau_CH4, tau_N2)
...
PhiPhiE_14SolarMean = np.loadtxt('phiphiElamb_14SolarMean_1dr.ascii')
phi_0 = PhiPhiE_14SolarMean[0]
phiE_0 = PhiPhiE_14SolarMean[1]
Exobase_14SolarMean = np.loadtxt('ExobaseVals_14SolarMean_1dr.ascii')
i0_x = int(Exobase_14SolarMean[0])
NumTempRadVel_14SolarMean = np.loadtxt('NumTempRadVel_14SolarMean_1dr.ascii')
n0 = NumTempRadVel_14SolarMean[0,:]
r0 = NumTempRadVel_14SolarMean[1,:]
T0 = NumTempRadVel_14SolarMean[2,:]
Convergence_14SolarMean = np.loadtxt('Convergence_14SolarMean_1dr.ascii')
TIME_14SolarMean = Convergence_14SolarMean[0]
iteration_14SolarMean = Convergence_14SolarMean[1]


i1_x, n1_x, r1_x, T1_x, n1, r1, T1, u1, phi_1, phiE_1, lamb_x = \
    PlutoProj.convergence_iteration_145SolarMean(i0_x, n0, r0, T0, phi_0, phiE_0)
...
while np.fabs(100.*(phi_new-phi_old)/(phi_old)) > 3.0:
    phi_avg = ( (1-w) * phi_old_avg) + (w * phi_new)
    phiE_avg = ( (1-w) * phiE_old_avg) + (w * phiE_new)
    ...
    PlutoProj.convergence_iteration_145SolarMean(ix_old, n_old, r_old, T_old, phi_old_avg, phiE_old_avg)
...
TIME = TIME + TIME_14SolarMean

iteration = iteration + iteration_14SolarMean


i_x_145SolarMean = i_x
n_x_145SolarMean = n_x
r_x_145SolarMean = r_x
T_x_145SolarMean = T_x
n_145SolarMean = n
r_145SolarMean = r
T_145SolarMean = T
u_145SolarMean = u
phi_145SolarMean = phi_new
phiE_145SolarMean = phiE_new
lamb_x_145SolarMean = lamb_x

Convergence_Results = [TIME, iteration]
np.savetxt('Convergence_145SolarMean_1dr.ascii', Convergence_Results)

NumTempRadVel_Results = [n_145SolarMean, T_145SolarMean, r_145SolarMean, u_145SolarMean]
phiphiElamb_Results = [phi_145SolarMean, phiE_145SolarMean, lamb_x_145SolarMean]
Exobase_Results = [i_x_145SolarMean, n_x_145SolarMean, r_x_145SolarMean, T_x_145SolarMean]
np.savetxt('NumTempRadVel_145SolarMean_1dr.ascii', NumTempRadVel_Results)
np.savetxt('phiphiElamb_145SolarMean_1dr.ascii', phiphiElamb_Results)
np.savetxt('ExobaseVals_145SolarMean_1dr.ascii', Exobase_Results)